var _background = {
  ...
  rander: function () {
    ...
    for (var i = self.minNumber, j = self.maxNumber; i <= j; i++) {
      innerHTML +=
        '<div class="lotto-number" data-check="N" data-number="' + i + '">';
  ...
  execLotto: function () {
    ...
    while (self.fixedNumbers.length + numbers.length < 6) {
      var number = Math.floor(
        Math.random() * (self.maxNumber - self.minNumber) + self.minNumber
      );
  ...
  renderResult: function () {
    ...
    for (var i = 0, j = numbers.length; i < j; i++) {
      ...
      innerHTML +=
        '<div class="lotto-ball ' + className + '">' + number + "</div>";
    }
  ...
  checked: function () {
    ...
    if (
      self.fixedNumbers.length +
        self.exclusionNumbers.length +
        self.execNumber.length >
      0
    ) {
